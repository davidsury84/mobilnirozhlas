...
function mount(host) {
  ...
  async function handle(req, res) {
    ...
    if (p.startsWith('/konstrukce/nahled/') || p.startsWith('/api/konstrukce/nahled/') || p.startsWith('/konstrukce/prehled/') || p.startsWith('/api/konstrukce/prehled/')) {
  ...
  function publicShape(d, z, me) {
    ...
    return {
      ...
      link: z.link ? { active: z.link.active, expiresAt: z.link.expiresAt, url: '/konstrukce/nahled/' + z.link.token, hasPin: !!z.link.pin, accesses: (z.link.accesses || []).length,
        ...
        prehledUrl: prehledPath(d, z.kontaktEmail),   // stálý přehled všech výkresů tohoto klienta
  ...
  const PLANY = {
    ...
    // Supíkovice vedou ČVZ v jedné buňce („26S177" ve sloupci B, A = křestní jméno konstruktéra)
    // a čísla mají předvyplněná dopředu — nová zakázka dostane číslo prvního volného řádku.
    // POZOR: list „výrobní čísla" v téže tabulce je jiná řada (výrobní čísla kusů, 26S5xx) — sem nepatří.
    supikovice: { nazev: 'PLÁN VÝROBY SUPÍKOVICE', kod: 'S', hlavicka: 1,
      sheet: process.env.KONSTRUKCE_PLAN_SHEET_SUPIKOVICE || '19ZskN-LJssZvGwRuEJ7rjfn19dsQqzZs-2z_sbXi-yY',
      list: () => 'zakázky Supíkovice',
      cvzVJedneBunce: true, bezHlavicky: { konstrukter: 0 },
      // zvyklosti listu: objem v názvu výrobku („ABR-DSD 39cbm"), plechy za rozměry („…x2350/53"),
      // RAL bez předpony („6004"), ve sloupci Kontakt celé jméno obchodníka
      objemDoVyrobku: true, tlDoRozmeru: true, ralBezPredpony: true, kontaktCeleJmeno: true },
  };
  ...
  const PLAN_SLOUPCE = {
    vyrobek: ['vyrobek'],
    rozmery: ['rozmery'],
    tl: ['tl. v mm', 'tl v mm', 'tl.', 'thickness mm'],
    objem: ['objem'],
    napojeniLem: ['napojeni/lem', 'napojeni / lem', 'napojeni'],
  ...
  function planMapa(hlavicka, cfg) {
    cfg = cfg || {};
    ...
    Object.assign(idx, cfg.bezHlavicky || {});   // sloupce bez hlavičky (Supíkovice: A = konstruktér)
    if (cfg.cvzVJedneBunce) { const i = h.indexOf('cvz'); idx.cvz = i >= 0 ? i : 1; }   // „26S177" v jedné buňce
  ...
  function planHodnoty(d, z, pref, cislo, opt, cfg) {
    ...
    return {
      prefix: pref, cislo: num, cvz: pref + num,
      vyrobek, rozmery,
  ...
  async function planCil(cfg, rok) {
    ...
    const j = await host.sheets.read(cfg.sheet, "'" + list + "'!A1:BZ5000");
    const rows = j.values || [];
    const hl = rows[cfg.hlavicka - 1] || [];
    const mapa = planMapa(hl, cfg);
    const pref = String(rok).slice(2) + cfg.kod;
    const jedna = !!cfg.cvzVJedneBunce, iCvz = jedna ? mapa.cvz : 1;
    // číslo řady v řádku: A/B („26C" | „271"), nebo jedna buňka („26S177", i „26S-177")
    const cisloRadku = r => {
      r = r || [];
      if (!jedna) return String(r[0] || '').trim() === pref ? parseInt(String(r[1] || '').trim(), 10) : NaN;
      const m = String(r[iCvz] || '').trim().match(/^(\d{2}[A-Z])\s*-?\s*(\d{3,4})$/);
      return (m && m[1] === pref) ? +m[2] : NaN;
    };
    // „má data" = něco kromě samotného čísla řady (Bruntál má předvyplněné prefixy, Supíkovice celá čísla)
    const maData = jedna
      ? (r => (r || []).some((x, i) => i !== iCvz && String(x || '').trim()))
      : (r => (r || []).slice(1).some(x => String(x || '').trim()));
    let max = 0, maxData = 0, posledniData = cfg.hlavicka - 1;
    for (let i = cfg.hlavicka; i < rows.length; i++) {
      const r = rows[i] || [];
      const n = cisloRadku(r);
      if (Number.isFinite(n)) { if (n > max) max = n; if (maData(r) && n > maxData) maxData = n; }
    ...
    // Supíkovice: číslo předvyplněné v cílovém řádku — bere se, jen když je nad posledním použitým
    const pv = jedna ? cisloRadku(rows[cil - 1]) : NaN;
    return { list, mapa, pref, max, maxData, cil, predvyplneno: (Number.isFinite(pv) && pv > maxData) ? pv : 0,
      sirka: Math.max(hl.length, ...Object.values(mapa).map(i => i + 1)) };
  }
  ...
  function planDalsi(d, cfg, c) {
    ...
    // Supíkovice: v cílovém řádku už číslo připravené je — bereme to, které tam ředitel předvyplnil
    if (c.predvyplneno && c.predvyplneno > nase) return c.predvyplneno;
    ...
    switch (action) {
      ...
      case 'odeslat-klientovi': { // obchodník odešle veřejný náhled (ručně, s možností upravit text)
        ...
        if (z.stav === 'klient') {
          ...
          const base2 = host.baseUrl ? host.baseUrl(req) : '';
          const url2 = base2 + '/konstrukce/nahled/' + z.link.token;
          const lang2 = LANGS_KLIENT.includes(String(b.lang || '')) ? String(b.lang) : (z.link.lang || 'cs');
          // druhý odkaz: přehled všech výkresů klienta, jen když jich na rozhodnutí čeká víc
          const rec2 = prehledPro(d, z.kontaktEmail || komu2, lang2);
          const vice2 = klientoviCekajici(d, z.kontaktEmail || komu2, z.id).length >= 1;
          const vzor2 = b.pripominka ? (REMIND_TXT[lang2] || REMIND_TXT.cs) : null;
          const text2 = dosadOdkazy(b.text || (vzor2 ? vzor2.t.replace('{C}', z.cislo).replace('{Z}', z.zakaznik) : '{ODKAZ}'), url2, rec2 ? base2 + '/konstrukce/prehled/' + rec2.token : '', vice2);
          const subj2 = String(b.subject || ((vzor2 ? vzor2.p : 'Výkres ke schválení') + ' · ' + z.cislo));
          if (b.pripominka) {
          ...
          json(res, 200, { ok: true, url: url2, znovu: true, pripominka: !!b.pripominka, prehledUrl: rec2 ? '/konstrukce/prehled/' + rec2.token : '', vice: vice2 });
          return true;
        }
        ...
        // jazyk náhledu i e-mailu — klient nemusí umět česky
        const lang = LANGS_KLIENT.includes(String(b.lang || '')) ? String(b.lang) : 'cs';
        z.link = { token, active: true, createdAt: Date.now(), expiresAt: addDaysCal(Date.now(), t.linkValidDays || 30), pin: b.pin ? String(b.pin).slice(0, 12) : '', accesses: [], lang };
        enterState(d, z, 'klient');
        audit(z, me.email, 'Odesláno klientovi', 'odkaz platí do ' + fmtDate(z.link.expiresAt) + (lang !== 'cs' ? (' · jazyk ' + lang.toUpperCase()) : ''));
        ...
        // druhý odkaz do e-mailu: přehled všech výkresů klienta čekajících na rozhodnutí
        // (stálý odkaz na e-mail klienta; řádek se vynechá, když čeká jen tento jeden)
        const vice = klientoviCekajici(d, z.kontaktEmail, z.id).length >= 1;
        const rec = prehledPro(d, z.kontaktEmail, lang);
        const prehledUrl = rec ? base + '/konstrukce/prehled/' + rec.token : '';
        const text = dosadOdkazy(b.text ? String(b.text) : defaultText, url, prehledUrl, vice);
        ...
        if (b.bezEmailu) {
          audit(z, me.email, 'Odkaz pro klienta vytvořen', 'e-mail si obchodník posílá sám' + (lang !== 'cs' ? (' · jazyk ' + lang.toUpperCase()) : ''));
          save(d);
          json(res, 200, { ok: true, url, text, subject, bezEmailu: true, prehledUrl: rec ? '/konstrukce/prehled/' + rec.token : '', vice });
          return true;
        }
        save(d);
        ...
        save(d);
        json(res, 200, { ok: true, url, prehledUrl: rec ? '/konstrukce/prehled/' + rec.token : '', vice });
        return true;
      }
  ...
  async function apiZadani(req, res) {
    ...
    // Technická změna po hotovém výkresu = výkres už neplatí. Zakázka se proto sama
    // vrátí konstruktérovi jako revize (nová verze) — e-mail by se ztratil, úkol ne.
    // Kontakty, termín nebo číslo poptávky výkres nemění → jen upozornění, stav zůstává.
    const technicke = zmeny.filter(x => x.k === 'typKey' || !HLAV_POLE.some(([k]) => k === x.k));
    let revize = null;
    if (technicke.length && maVykresKonstrukce(z) && ['kontrola', 'obchodnik', 'klient', 'schvaleno'].includes(z.stav)) {
      const puvodni = (STAV[z.stav] || {}).label || z.stav;
      lockDraft(z);                                  // co bylo nakresleno, zůstává jako historie
      const cur = CURRENT_V(z);
      z.revisionCount = (z.revisionCount || 0) + 1;
      const nv = { v: (cur ? cur.v : 0) + 1, author: z.assignedTo || '', createdAt: now, locked: false };
      z.versions.push(nv);
      z.clientDecision = null;                       // případné schválení staré verze už neplatí
      z.revizeDuvod = 'zmena-zadani';                // klient na odkazu vidí „zadání upraveno", ne „připomínky přijali"
      enterState(d, z, 'revize');
      revize = { v: nv.v, odkud: puvodni };
      audit(z, 'systém', 'Změna zadání po zkreslení → revize v' + nv.v,
        'z kroku „' + puvodni + '" zpět konstruktérovi; změněno: ' + technicke.map(x => x.label).join(', '));
    }
    const komu = new Set();
    ...
    komu.delete((me.email || '').toLowerCase());
    const kam = revize ? (' — výkres v' + (revize.v - 1) + ' neplatí, zakázka se vrátila konstruktérovi k překreslení (revize v' + revize.v + ')') : '';
    for (const em of komu) notify(d, em, 'Změna zadání ' + cis + ' (' + zmeny.length + '×) — ' + me.name + kam, z.id);
    save(d);
    const seznam = zmeny.map(x => '• ' + x.label + ': ' + (x.z || '—') + '  →  ' + (x.na || '—')).join('\n');
    const dovetek = revize
      ? ('\n\nVÝKRES JE K PŘEKRESLENÍ: zadání se změnilo technicky, proto se zakázka z kroku „' + revize.odkud
         + '" vrátila konstruktérovi do kroku Revize (nová verze v' + revize.v + (z.deadline ? (', termín ' + fmtDate(z.deadline)) : '') + ').'
         + ((z.link && z.link.active) ? ' Klient na svém odkazu vidí, že se výkres přepracovává — starou verzi už schválit nemůže. Po zkreslení projde zakázka kontrolou a obchodník ji klientovi pošle znovu.' : ''))
    ...
    for (const em of komu) {
      mail(em, 'Změna zadání · ' + cis + (revize ? ' · K PŘEKRESLENÍ' : ''),
        'Zadání zakázky ' + cis + ' (' + z.zakaznik + ') bylo upraveno.\n\nKdo: ' + me.name + '\nDůvod: ' + duvod
        + '\n\nZměny:\n' + seznam + dovetek,
        z, { stitek: revize ? 'ZMĚNA ZADÁNÍ — K PŘEKRESLENÍ' : 'ZMĚNA ZADÁNÍ', stitekBarva: '#b3261e' });
    }
    json(res, 200, { ok: true, zmeny: zmeny.length, revize: revize ? revize.v : 0 });
    return true;
  }
  ...
  async function handlePublic(req, res, u, p) {
    ...
    // přehled všech výkresů jednoho klienta (stálý odkaz)
    const mPre = /^\/konstrukce\/prehled\/([a-f0-9]{32,64})\/?$/.exec(p);
    if (mPre && req.method === 'GET') { htmlOut(res, 200, PREHLED_HTML); return true; }
    const mPreData = /^\/api\/konstrukce\/prehled\/([a-f0-9]{32,64})$/.exec(p);
    if (mPreData && req.method === 'GET') return apiPrehledData(req, res, mPreData[1], (u && u.query) || {});
  ...
  // Přehled klienta: všechny jeho výkresy čekající na rozhodnutí + co už rozhodl (90 dní).
  function apiPrehledData(req, res, token, q) {
    const d = load();
    const rec = najdiPrehled(d, token);
    if (!rec) { json(res, 410, { chyba: 'Odkaz není platný.' }); return true; }
    if (!Array.isArray(rec.accesses)) rec.accesses = [];
    rec.accesses.push({ at: Date.now(), ip: clientIp(req), ua: kratkyUA(req) });
    if (rec.accesses.length > 100) rec.accesses.splice(0, rec.accesses.length - 100);
    save(d);
    const k = rec.email;
    const vse = d.zakazky.filter(z => z.link && z.link.token && emailKlic(z.kontaktEmail) === k);
    const cek = vse.filter(z => z.stav === 'klient' && linkOk(z)).sort((a, b) => (a.link.createdAt || 0) - (b.link.createdAt || 0));
    const hranice = Date.now() - 90 * 864e5;
    const kdy = z => (z.clientDecision ? z.clientDecision.at : z.stepStartedAt) || 0;
    const vyr = vse.filter(z => !cek.includes(z) && ((z.clientDecision && z.clientDecision.at > hranice) || (z.stav === 'revize' && (z.stepStartedAt || 0) > hranice)))
      .sort((a, b) => kdy(b) - kdy(a));
    const posl = vse.slice().sort((a, b) => (b.link.createdAt || 0) - (a.link.createdAt || 0))[0] || null;
    const lang = LANGS_KLIENT.includes(String(q.lang || '')) ? String(q.lang) : (rec.lang || 'cs');
    json(res, 200, {
      lang, kontakt: posl ? (posl.kontakt || '') : '', zakaznik: posl ? (posl.zakaznik || '') : '',
      cekajici: cek.map(z => ({ cislo: z.cislo, typName: typeOf(d, z.typKey).name, verze: (CURRENT_V(z) || {}).v || null, odeslano: z.link.createdAt || null, url: '/konstrukce/nahled/' + z.link.token })),
      vyrizene: vyr.map(z => ({ cislo: z.cislo, typName: typeOf(d, z.typKey).name, stav: z.clientDecision ? z.clientDecision.action : 'revize', at: kdy(z), url: '/konstrukce/nahled/' + z.link.token })),
      obchodnik: posl && posl.obchodnikEmail ? { name: empName(posl.obchodnikEmail), email: posl.obchodnikEmail } : null,
    });
    return true;
  }
  ...
  // ---- přehled klienta: jeden stálý odkaz na všechny jeho výkresy čekající na rozhodnutí ----
  // Vázaný na e-mail kontaktní osoby; vzniká při prvním odeslání a dál se nemění.
  const emailKlic = (e) => String(e || '').trim().toLowerCase();
  const LANGS_KLIENT = ['cs', 'en', 'de', 'pl', 'ua', 'ro', 'fr'];
  function prehledPro(d, email, lang) {
    const k = emailKlic(email); if (!k) return null;
    if (!d.klientPrehled || typeof d.klientPrehled !== 'object') d.klientPrehled = {};
    let r = d.klientPrehled[k];
    if (!r) { r = { email: k, token: crypto.randomBytes(16).toString('hex'), createdAt: Date.now(), accesses: [] }; d.klientPrehled[k] = r; }
    if (lang) r.lang = lang;
    return r;
  }
  function najdiPrehled(d, token) {
    for (const r of Object.values(d.klientPrehled || {})) if (r && r.token === token) return r;
    return null;
  }
  function prehledPath(d, email) {
    const r = d.klientPrehled && d.klientPrehled[emailKlic(email)];
    return r ? '/konstrukce/prehled/' + r.token : '';
  }
  // ostatní výkresy téhož klienta (dle e-mailu), které právě čekají na jeho rozhodnutí
  function klientoviCekajici(d, email, krome) {
    const k = emailKlic(email); if (!k) return [];
    return d.zakazky.filter(z => z.id !== krome && z.stav === 'klient' && linkOk(z) && emailKlic(z.kontaktEmail) === k);
  }
  // {ODKAZ} = tento výkres, {PREHLED} = přehled všech čekajících. Řádek s přehledem má
  // smysl, jen když klient čeká na víc výkresů najednou — jinak se z textu vynechá.
  function dosadOdkazy(text, url, prehledUrl, vice) {
    let t = String(text || '');
    if (!vice || !prehledUrl) t = t.split('\n').filter(l => l.indexOf('{PREHLED}') < 0).join('\n');
    return t.replace(/\{ODKAZ\}/g, url).replace(/\{PREHLED\}/g, prehledUrl || '');
  }
  // Připomínka klientovi (ruční i automatická po N dnech) — v jazyce, ve kterém dostal odkaz.
  const REMIND_TXT = {
    cs: { p: 'Připomínka: výkres ke schválení', t: 'Dobrý den,\n\ndovolujeme si připomenout, že výkres k zakázce {C} ({Z}) stále čeká na Vaše schválení.\nProhlédnout a schválit jej můžete zde:\n{ODKAZ}\nPřehled všech Vašich výkresů čekajících na rozhodnutí: {PREHLED}\n\nDěkujeme a jsme s pozdravem' },
    en: { p: 'Reminder: drawing awaiting your approval', t: 'Dear Sir or Madam,\n\nthis is a friendly reminder that the drawing for order {C} ({Z}) is still awaiting your approval.\nYou can review and approve it here:\n{ODKAZ}\nOverview of all your drawings awaiting a decision: {PREHLED}\n\nThank you and kind regards' },
    de: { p: 'Erinnerung: Zeichnung wartet auf Ihre Freigabe', t: 'Sehr geehrte Damen und Herren,\n\nwir möchten Sie daran erinnern, dass die Zeichnung zum Auftrag {C} ({Z}) noch auf Ihre Freigabe wartet.\nSie können sie hier ansehen und freigeben:\n{ODKAZ}\nÜbersicht aller Ihrer Zeichnungen, die auf eine Entscheidung warten: {PREHLED}\n\nVielen Dank und freundliche Grüße' },
    pl: { p: 'Przypomnienie: rysunek czeka na zatwierdzenie', t: 'Dzień dobry,\n\nuprzejmie przypominamy, że rysunek do zlecenia {C} ({Z}) nadal czeka na Państwa zatwierdzenie.\nMożna go obejrzeć i zatwierdzić tutaj:\n{ODKAZ}\nPrzegląd wszystkich Państwa rysunków oczekujących na decyzję: {PREHLED}\n\nDziękujemy i pozdrawiamy' },
    ua: { p: 'Нагадування: креслення очікує на затвердження', t: 'Доброго дня,\n\nнагадуємо, що креслення до замовлення {C} ({Z}) досі очікує на ваше затвердження.\nПереглянути та затвердити його можна тут:\n{ODKAZ}\nОгляд усіх ваших креслень, що очікують на рішення: {PREHLED}\n\nДякуємо, з повагою' },
    ro: { p: 'Reamintire: desenul așteaptă aprobarea dumneavoastră', t: 'Bună ziua,\n\nvă reamintim că desenul pentru comanda {C} ({Z}) așteaptă în continuare aprobarea dumneavoastră.\nÎl puteți vedea și aproba aici:\n{ODKAZ}\nLista tuturor desenelor dumneavoastră care așteaptă o decizie: {PREHLED}\n\nVă mulțumim, cu stimă' },
    fr: { p: 'Rappel : dessin en attente de votre approbation', t: 'Bonjour,\n\nnous nous permettons de vous rappeler que le dessin de la commande {C} ({Z}) attend toujours votre approbation.\nVous pouvez le consulter et l’approuver ici :\n{ODKAZ}\nListe de tous vos dessins en attente de décision : {PREHLED}\n\nMerci et cordialement' },
  };
  ...
  function apiPublicData(req, res, token) {
    ...
    json(res, 200, {
      ...
      kontakt: z.kontakt || '',
      lang: (z.link && z.link.lang) || 'cs',   // jazyk zvolený při odeslání
      version: cur ? cur.v : null, versionCount: z.versions.length,
      hasPdf: !!(cur && cur.pdf),
      revize: z.stav === 'revize',   // po připomínkách: chystá se nová verze
      revizeZadani: z.stav === 'revize' && z.revizeDuvod === 'zmena-zadani',   // …ne kvůli připomínkám, ale změně zadání
      // klient čeká na víc výkresů najednou → nahoře odkaz na přehled všech
      prehled: (() => { const n = klientoviCekajici(d, z.kontaktEmail, z.id).length; const pth = prehledPath(d, z.kontaktEmail); return (n && pth) ? { url: pth, dalsi: n } : null; })(),
      ...
      if (z.stav === 'klient' && z.stepStartedAt) {
        ...
        if (remind1 > 0 && bdays >= remind1 && !z.esc.klient5) {
          ...
          if (z.kontaktEmail && z.link && z.link.active) {
            ...
            const lr = z.link.lang || 'cs', vz = REMIND_TXT[lr] || REMIND_TXT.cs;
            const rec = prehledPro(d, z.kontaktEmail, lr);
            const vice = klientoviCekajici(d, z.kontaktEmail, z.id).length >= 1;
            mail(z.kontaktEmail, vz.p + ' · ' + z.cislo,
              dosadOdkazy(vz.t.replace('{C}', z.cislo).replace('{Z}', z.zakaznik), base + '/konstrukce/nahled/' + z.link.token, rec ? base + '/konstrukce/prehled/' + rec.token : '', vice));
            if (!Array.isArray(z.link.pripominky)) z.link.pripominky = [];
            z.link.pripominky.push({ at: Date.now(), komu: z.kontaktEmail, by: 'systém' });
          }
        }
...
// Slovník klientských stránek (náhled i přehled): klíč = čeština, pole = [en, de, pl, ua, ro, fr]
const NL_LINE = `var NL={"Zadání zakázky bylo upraveno — výkres se přepracovává.":["The order specification has been changed — the drawing is being reworked.","Die Auftragsvorgaben wurden geändert — die Zeichnung wird überarbeitet.","Specyfikacja zamówienia została zmieniona — rysunek jest przerabiany.","Специфікацію замовлення змінено — креслення доопрацьовується.","Specificația comenzii a fost modificată — desenul este refăcut.","Les spécifications de la commande ont été modifiées — le dessin est en cours de révision."],"Náhled výkresu ke schválení":["Drawing for approval","Zeichnung zur Freigabe","Rysunek do zatwierdzenia","Креслення на затвердження","Desen spre aprobare","Aperçu du dessin à approuver"],"Načítám…":["Loading…","Wird geladen…","Wczytywanie…","Завантаження…","Se încarcă…","Chargement…"],"Odkaz není platný.":["This link is not valid.","Dieser Link ist ungültig.","Link jest nieprawidłowy.","Посилання недійсне.","Linkul nu este valid.","Ce lien n’est pas valide."],"Nepodařilo se načíst náhled.":["The preview could not be loaded.","Die Vorschau konnte nicht geladen werden.","Nie udało się wczytać podglądu.","Не вдалося завантажити перегляд.","Previzualizarea nu a putut fi încărcată.","Impossible de charger l’aperçu."],"Nepodařilo se odeslat.":["Sending failed.","Senden fehlgeschlagen.","Nie udało się wysłać.","Не вдалося надіслати.","Trimiterea a eșuat.","L’envoi a échoué."],"Tento náhled je chráněn PINem, který jste dostali zvlášť.":["This preview is protected by a PIN you received separately.","Diese Vorschau ist durch eine separat zugesandte PIN geschützt.","Ten podgląd jest chroniony PIN-em przesłanym osobno.","Цей перегляд захищено PIN-кодом, який ви отримали окремо.","Această previzualizare este protejată printr-un PIN primit separat.","Cet aperçu est protégé par un code PIN que vous avez reçu séparément."],"Pokračovat":["Continue","Weiter","Dalej","Продовжити","Continuă","Continuer"],"PDF výkresu není k dispozici.":["The drawing PDF is not available.","Das Zeichnungs-PDF ist nicht verfügbar.","PDF rysunku jest niedostępny.","PDF креслення недоступний.","PDF-ul desenului nu este disponibil.","Le PDF du dessin n’est pas disponible."],"Výkres":["Drawing","Zeichnung","Rysunek","Креслення","Desen","Dessin"],"Zákazník:":["Customer:","Kunde:","Klient:","Замовник:","Client:","Client :"],"Verze:":["Version:","Version:","Wersja:","Версія:","Versiune:","Version :"],"NÁHLED":["PREVIEW","VORSCHAU","PODGLĄD","ПЕРЕГЛЯД","PREVIZUALIZARE","APERÇU"],"Prohlédněte si výkres a zvolte, jak chcete pokračovat:":["Review the drawing and choose how to proceed:","Prüfen Sie die Zeichnung und wählen Sie, wie es weitergehen soll:","Sprawdź rysunek i wybierz, jak chcesz kontynuować:","Перегляньте креслення та оберіть, як продовжити:","Verificați desenul și alegeți cum doriți să continuați:","Consultez le dessin et choisissez comment poursuivre :"],"✓ Schválit":["✓ Approve","✓ Freigeben","✓ Zatwierdź","✓ Затвердити","✓ Aprobă","✓ Approuver"],"✎ Poslat připomínky":["✎ Send comments","✎ Anmerkungen senden","✎ Wyślij uwagi","✎ Надіслати зауваження","✎ Trimite observații","✎ Envoyer des remarques"],"✕ Zamítnout":["✕ Reject","✕ Ablehnen","✕ Odrzuć","✕ Відхилити","✕ Respinge","✕ Refuser"],"Rozhodujete jako":["You are deciding as","Sie entscheiden als","Decydujesz jako","Ви вирішуєте як","Decideți ca","Vous décidez en tant que"],"rozhoduje někdo jiný?":["someone else deciding?","entscheidet jemand anderes?","decyduje ktoś inny?","вирішує хтось інший?","decide altcineva?","quelqu’un d’autre décide ?"],"Vaše jméno":["Your name","Ihr Name","Twoje imię i nazwisko","Ваше імʼя","Numele dumneavoastră","Votre nom"],"Jméno a příjmení":["First and last name","Vor- und Nachname","Imię i nazwisko","Імʼя та прізвище","Nume și prenume","Prénom et nom"],"Potvrzuji, že výkres odpovídá objednávce a schvaluji jej k výrobě.":["I confirm the drawing matches the order and approve it for production.","Ich bestätige, dass die Zeichnung der Bestellung entspricht, und gebe sie für die Fertigung frei.","Potwierdzam, że rysunek jest zgodny z zamówieniem i zatwierdzam go do produkcji.","Підтверджую, що креслення відповідає замовленню, і затверджую його у виробництво.","Confirm că desenul corespunde comenzii și îl aprob pentru producție.","Je confirme que le dessin correspond à la commande et je l’approuve pour la production."],"Schválit výkres":["Approve the drawing","Zeichnung freigeben","Zatwierdź rysunek","Затвердити креслення","Aprobă desenul","Approuver le dessin"],"Připomínky k výkresu":["Comments on the drawing","Anmerkungen zur Zeichnung","Uwagi do rysunku","Зауваження до креслення","Observații la desen","Remarques sur le dessin"],"Popište, co je třeba upravit…":["Describe what needs to change…","Beschreiben Sie, was geändert werden soll…","Opisz, co należy poprawić…","Опишіть, що потрібно змінити…","Descrieți ce trebuie modificat…","Décrivez ce qu’il faut modifier…"],"Odeslat připomínky":["Send comments","Anmerkungen senden","Wyślij uwagi","Надіслати зауваження","Trimite observațiile","Envoyer les remarques"],"Důvod zamítnutí":["Reason for rejection","Grund der Ablehnung","Powód odrzucenia","Причина відхилення","Motivul respingerii","Motif du refus"],"Uveďte prosím důvod…":["Please state the reason…","Bitte geben Sie den Grund an…","Podaj proszę powód…","Вкажіть, будь ласка, причину…","Vă rugăm să indicați motivul…","Veuillez indiquer le motif…"],"Zamítnout výkres":["Reject the drawing","Zeichnung ablehnen","Odrzuć rysunek","Відхилити креслення","Respinge desenul","Refuser le dessin"],"✓ Výkres byl schválen a předán do výroby.":["✓ The drawing has been approved and released to production.","✓ Die Zeichnung wurde freigegeben und in die Fertigung übergeben.","✓ Rysunek został zatwierdzony i przekazany do produkcji.","✓ Креслення затверджено та передано у виробництво.","✓ Desenul a fost aprobat și trimis în producție.","✓ Le dessin a été approuvé et transmis à la production."],"Odkaz zůstává platný — výkres si tu můžete kdykoli znovu prohlédnout.":["The link stays valid — you can view the drawing here at any time.","Der Link bleibt gültig — Sie können die Zeichnung hier jederzeit ansehen.","Link pozostaje ważny — rysunek możesz tu obejrzeć w każdej chwili.","Посилання залишається дійсним — креслення можна переглянути тут будь-коли.","Linkul rămâne valabil — puteți vedea desenul aici oricând.","Le lien reste valide — vous pouvez consulter le dessin ici à tout moment."],"✕ Výkres byl zamítnut.":["✕ The drawing has been rejected.","✕ Die Zeichnung wurde abgelehnt.","✕ Rysunek został odrzucony.","✕ Креслення відхилено.","✕ Desenul a fost respins.","✕ Le dessin a été refusé."],"Ozve se Vám náš obchodník.":["Our sales representative will contact you.","Unser Vertriebsmitarbeiter meldet sich bei Ihnen.","Skontaktuje się z Tobą nasz handlowiec.","З вами звʼяжеться наш менеджер.","Agentul nostru de vânzări vă va contacta.","Notre commercial vous contactera."],"Připomínky jsme přijali.":["We have received your comments.","Wir haben Ihre Anmerkungen erhalten.","Otrzymaliśmy Twoje uwagi.","Ми отримали ваші зауваження.","Am primit observațiile dumneavoastră.","Nous avons bien reçu vos remarques."],"Konstrukce zapracovává úpravy — jakmile bude nová verze hotová, pošleme Vám ji ke schválení.":["Engineering is making the changes — we will send you the new version for approval as soon as it is ready.","Die Konstruktion arbeitet die Änderungen ein — sobald die neue Version fertig ist, senden wir sie Ihnen zur Freigabe.","Konstrukcja wprowadza zmiany — gdy nowa wersja będzie gotowa, prześlemy ją do zatwierdzenia.","Конструкторський відділ вносить зміни — щойно нова версія буде готова, надішлемо її на затвердження.","Proiectarea implementează modificările — vă vom trimite noua versiune spre aprobare imediat ce este gata.","Le bureau d’études intègre les modifications — dès que la nouvelle version sera prête, nous vous l’enverrons pour approbation."],"Zakázka byla stornována.":["The order has been cancelled.","Der Auftrag wurde storniert.","Zlecenie zostało anulowane.","Замовлення скасовано.","Comanda a fost anulată.","La commande a été annulée."],"V případě dotazu se prosím obraťte na svého obchodníka.":["If you have any questions, please contact your sales representative.","Bei Fragen wenden Sie sich bitte an Ihren Vertriebsmitarbeiter.","W razie pytań skontaktuj się ze swoim handlowcem.","У разі запитань звертайтеся до свого менеджера.","Pentru întrebări, contactați agentul dumneavoastră de vânzări.","Pour toute question, veuillez contacter votre commercial."],"Zabezpečený náhled · dokument slouží jen ke schválení, nešiřte prosím odkaz dál.":["Secure preview · the document is for approval only, please do not share the link.","Geschützte Vorschau · das Dokument dient nur der Freigabe, bitte teilen Sie den Link nicht.","Zabezpieczony podgląd · dokument służy wyłącznie do zatwierdzenia, prosimy nie udostępniać linku.","Захищений перегляд · документ призначений лише для затвердження, не поширюйте посилання.","Previzualizare securizată · documentul servește doar aprobării, vă rugăm să nu distribuiți linkul.","Aperçu sécurisé · le document sert uniquement à l’approbation, merci de ne pas transmettre le lien."],"Komunikace":["Messages","Kommunikation","Komunikacja","Листування","Comunicare","Messages"],"Klient":["Customer","Kunde","Klient","Замовник","Client","Client"],"ACTS — železniční abroll kontejner":["ACTS — rail hook-lift container","ACTS — Abrollcontainer für die Bahn","ACTS — kolejowy kontener hakowy","ACTS — залізничний гаковий контейнер","ACTS — container feroviar cu cârlig","ACTS — conteneur ampliroll ferroviaire"],"AFO — kontejnery pro f. Hamo (skandinávský typ)":["AFO — containers for Hamo (Scandinavian type)","AFO — Container für Fa. Hamo (skandinavischer Typ)","AFO — kontenery dla f. Hamo (typ skandynawski)","AFO — контейнери для ф. Hamo (скандинавський тип)","AFO — containere pentru Hamo (tip scandinav)","AFO — conteneurs pour Hamo (type scandinave)"],"AFS — kontejnery v normě AFNOR":["AFS — containers to AFNOR standard","AFS — Container nach AFNOR","AFS — kontenery wg normy AFNOR","AFS — контейнери за нормою AFNOR","AFS — containere conform AFNOR","AFS — conteneurs à la norme AFNOR"],"ALST — provedení Alustahl (vybraní zákazníci)":["ALST — Alustahl design (selected customers)","ALST — Ausführung Alustahl (ausgewählte Kunden)","ALST — wykonanie Alustahl (wybrani klienci)","ALST — виконання Alustahl (окремі замовники)","ALST — execuție Alustahl (clienți selectați)","ALST — exécution Alustahl (clients sélectionnés)"],"ARC — kontejner na sklo":["ARC — glass container","ARC — Glascontainer","ARC — kontener na szkło","ARC — контейнер для скла","ARC — container pentru sticlă","ARC — conteneur à verre"],"BS — provedení s podélnými prolisy (Veolia France)":["BS — design with longitudinal beads (Veolia France)","BS — Ausführung mit Längssicken (Veolia France)","BS — wykonanie z przetłoczeniami wzdłużnymi (Veolia France)","BS — виконання з поздовжніми зигами (Veolia France)","BS — execuție cu nervuri longitudinale (Veolia France)","BS — exécution à nervures longitudinales (Veolia France)"],"CITY — uzavřený městský kontejner (hákový)":["CITY — enclosed municipal container (hook-lift)","CITY — geschlossener Stadtcontainer (Abroll)","CITY — zamknięty kontener miejski (hakowy)","CITY — закритий міський контейнер (гаковий)","CITY — container municipal închis (cu cârlig)","CITY — conteneur urbain fermé (ampliroll)"],"DSD — kontejnery v normě DIN (DIN 30722)":["DSD — containers to DIN standard (DIN 30722)","DSD — Container nach DIN (DIN 30722)","DSD — kontenery wg normy DIN (DIN 30722)","DSD — контейнери за нормою DIN (DIN 30722)","DSD — containere conform DIN (DIN 30722)","DSD — conteneurs à la norme DIN (DIN 30722)"],"ECL — kulaté bezvýztuhové provedení (pipe shape)":["ECL — round stiffener-free design (pipe shape)","ECL — runde versteifungsfreie Ausführung (Pipe Shape)","ECL — okrągłe wykonanie bez wzmocnień (pipe shape)","ECL — кругле виконання без ребер (pipe shape)","ECL — execuție rotundă fără rigidizări (pipe shape)","ECL — exécution ronde sans raidisseurs (pipe shape)"],"HBI — Hardox velké vypouklé (BIG & strong)":["HBI — Hardox large convex (BIG & strong)","HBI — Hardox groß gewölbt (BIG & strong)","HBI — Hardox duże wypukłe (BIG & strong)","HBI — Hardox великі опуклі (BIG & strong)","HBI — Hardox mari bombate (BIG & strong)","HBI — Hardox grand bombé (BIG & strong)"],"HBS — Hardox „halbšálen\\" (půlkulaté provedení)":["HBS — Hardox half-shell (semi-round design)","HBS — Hardox Halbschalen (halbrunde Ausführung)","HBS — Hardox półskorupowe (wykonanie półokrągłe)","HBS — Hardox напівкругле виконання","HBS — Hardox semirotund (execuție semicirculară)","HBS — Hardox demi-coque (exécution semi-ronde)"],"LWC — lehké provedení Hardox / Strenx 700":["LWC — lightweight Hardox / Strenx 700 design","LWC — Leichtausführung Hardox / Strenx 700","LWC — wykonanie lekkie Hardox / Strenx 700","LWC — легке виконання Hardox / Strenx 700","LWC — execuție ușoară Hardox / Strenx 700","LWC — exécution légère Hardox / Strenx 700"],"MULDA — skip / Absetzmulde (řetězová ramena)":["MULDA — skip / Absetzmulde (chain arms)","MULDE — Absetzmulde (Kettenarme)","MULDA — kontener KP / Absetzmulde (ramiona łańcuchowe)","MULDA — скіп / Absetzmulde (ланцюгові рамена)","MULDA — benă / Absetzmulde (brațe cu lanț)","MULDA — benne à chaînes / Absetzmulde (bras à chaînes)"],"NL — kontejnery pro holandský trh (Dutch type)":["NL — containers for the Dutch market (Dutch type)","NL — Container für den niederländischen Markt (Dutch type)","NL — kontenery na rynek holenderski (Dutch type)","NL — контейнери для нідерландського ринку (Dutch type)","NL — containere pentru piața olandeză (Dutch type)","NL — conteneurs pour le marché néerlandais (type Dutch)"],"PAL — plato s alu bočnicemi":["PAL — flatbed with aluminium sides","PAL — Plattform mit Alu-Seitenwänden","PAL — platforma z burtami aluminiowymi","PAL — платформа з алюмінієвими бортами","PAL — platformă cu obloane din aluminiu","PAL — plateau à ridelles aluminium"],"PAP — abroll s otvory na papír":["PAP — hook-lift with paper openings","PAP — Abrollcontainer mit Papieröffnungen","PAP — hakowy z otworami na papier","PAP — гаковий з отворами для паперу","PAP — cu cârlig cu orificii pentru hârtie","PAP — ampliroll avec ouvertures pour papier"],"PDK — plato, dřevěná podlaha, s klanicemi":["PDK — flatbed, wooden floor, with stakes","PDK — Plattform, Holzboden, mit Rungen","PDK — platforma, podłoga drewniana, z kłonicami","PDK — платформа, дерев’яна підлога, зі стійками","PDK — platformă, podea din lemn, cu țepuși","PDK — plateau, plancher bois, avec ranchers"],"POP — abroll s víky (Hausmüll)":["POP — hook-lift with lids (Hausmüll)","POP — Abrollcontainer mit Deckeln (Hausmüll)","POP — hakowy z pokrywami (Hausmüll)","POP — гаковий з кришками (Hausmüll)","POP — cu cârlig cu capace (Hausmüll)","POP — ampliroll à couvercles (Hausmüll)"],"PS — plato standard, ocelová podlaha":["PS — standard flatbed, steel floor","PS — Plattform Standard, Stahlboden","PS — platforma standard, podłoga stalowa","PS — платформа стандарт, сталева підлога","PS — platformă standard, podea din oțel","PS — plateau standard, plancher acier"],"PSK — plato, ocelová podlaha, s klanicemi":["PSK — flatbed, steel floor, with stakes","PSK — Plattform, Stahlboden, mit Rungen","PSK — platforma, podłoga stalowa, z kłonicami","PSK — платформа, сталева підлога, зі стійками","PSK — platformă, podea din oțel, cu țepuși","PSK — plateau, plancher acier, avec ranchers"],"PSN — plato se zadním nájezdem":["PSN — flatbed with rear ramp","PSN — Plattform mit Auffahrt hinten","PSN — platforma z najazdem tylnym","PSN — платформа із заднім заїздом","PSN — platformă cu rampă spate","PSN — plateau avec rampe arrière"],"PSNK — plato se zadním nájezdem a klanicemi":["PSNK — flatbed with rear ramp and stakes","PSNK — Plattform mit Auffahrt hinten und Rungen","PSNK — platforma z najazdem tylnym i kłonicami","PSNK — платформа із заднім заїздом і стійками","PSNK — platformă cu rampă spate și țepuși","PSNK — plateau avec rampe arrière et ranchers"],"PT — plato těžké provedení s prohlubní":["PT — heavy-duty flatbed with recess","PT — Plattform Schwerausführung mit Vertiefung","PT — platforma ciężka z zagłębieniem","PT — платформа важкого виконання із заглибленням","PT — platformă grea cu adâncitură","PT — plateau renforcé avec creux"],"RAM — rám ABR (frames)":["RAM — ABR frame (frames)","RAM — ABR-Rahmen (Frames)","RAM — rama ABR (frames)","RAM — рама ABR (frames)","RAM — cadru ABR (frames)","RAM — châssis ABR (frames)"],"SLD — kontejner na separovaný sběr":["SLD — separate collection container","SLD — Container für Getrenntsammlung","SLD — kontener do zbiórki selektywnej","SLD — контейнер для роздільного збору","SLD — container pentru colectare selectivă","SLD — conteneur pour collecte sélective"],"STH — stohovací kontejnery dle DIN":["STH — stackable containers to DIN","STH — stapelbare Container nach DIN","STH — kontenery sztaplowane wg DIN","STH — контейнери, що штабелюються, за DIN","STH — containere stivuibile conform DIN","STH — conteneurs gerbables selon DIN"],"SU — kontejner na separovaný sběr":["SU — separate collection container","SU — Container für Getrenntsammlung","SU — kontener do zbiórki selektywnej","SU — контейнер для роздільного збору","SU — container pentru colectare selectivă","SU — conteneur pour collecte sélective"],"USB / LSB — skladovací stohovací box":["USB / LSB — stackable storage box","USB / LSB — stapelbare Lagerbox","USB / LSB — skrzynia magazynowa sztaplowana","USB / LSB — складський бокс, що штабелюється","USB / LSB — cutie de depozitare stivuibilă","USB / LSB — caisse de stockage gerbable"],"WD — bezvýztuhové kontejnery v normě DIN":["WD — stiffener-free containers to DIN","WD — versteifungsfreie Container nach DIN","WD — kontenery bez wzmocnień wg DIN","WD — контейнери без ребер за DIN","WD — containere fără rigidizări conform DIN","WD — conteneurs sans raidisseurs à la norme DIN"],"WF — bezvýztuhové kontejnery v normě AFNOR":["WF — stiffener-free containers to AFNOR","WF — versteifungsfreie Container nach AFNOR","WF — kontenery bez wzmocnień wg AFNOR","WF — контейнери без ребер за AFNOR","WF — containere fără rigidizări conform AFNOR","WF — conteneurs sans raidisseurs à la norme AFNOR"],"HDC — Hardox korby tvaru U (typ à la GTS)":["HDC — Hardox U-shaped bodies (GTS type)","HDC — Hardox U-Mulden (Typ GTS)","HDC — skrzynie Hardox w kształcie U (typ GTS)","HDC — кузови Hardox U-подібні (тип GTS)","HDC — bene Hardox in forma de U (tip GTS)","HDC — bennes Hardox en U (type GTS)"],"TH — thermokontejner":["TH — thermo container","TH — Thermocontainer","TH — kontener termiczny","TH — термоконтейнер","TH — container termic","TH — conteneur isotherme"],"Máte další výkresy čekající na Vaše rozhodnutí:":["You have other drawings awaiting your decision:","Weitere Zeichnungen warten auf Ihre Entscheidung:","Masz kolejne rysunki oczekujące na decyzję:","У вас є інші креслення, що очікують на ваше рішення:","Aveți alte desene care așteaptă decizia dumneavoastră:","D’autres dessins attendent votre décision :"],"Zobrazit přehled všech výkresů →":["Show the overview of all drawings →","Übersicht aller Zeichnungen anzeigen →","Pokaż przegląd wszystkich rysunków →","Показати огляд усіх креслень →","Afișează lista tuturor desenelor →","Voir la liste de tous les dessins →"]};
var LANG='cs', LIDX={en:0,de:1,pl:2,ua:3,ro:4,fr:5};
function t(x){ if(LANG==='cs'||!x) return x; var e=NL[x]; var i=LIDX[LANG]; return (e&&e[i])||x; }`;
...
function render(j){
  KONTAKT=j.kontakt||'';
  LANG=j.lang||'cs';
  if(LANG!=='cs'){ document.documentElement.lang=(LANG==='ua'?'uk':LANG);
    document.title=t('Náhled výkresu ke schválení')+' · ELKOPLAST';
    var th=document.querySelector('.top .muted'); if(th)th.textContent=t('Náhled výkresu ke schválení');
    var tf=document.querySelector('.foot'); if(tf)tf.textContent=t('Zabezpečený náhled · dokument slouží jen ke schválení, nešiřte prosím odkaz dál.'); }
  var d=j.decided;
  var pdf=j.hasPdf?'<div class="pdfbox"><iframe src="'+esc(j.pdfUrl)+'#toolbar=1&navpanes=0"></iframe><div class="wm"><span>'+t('NÁHLED')+' · '+esc(j.zakaznik||'')+'</span></div></div>':'<p class="muted">'+t('PDF výkresu není k dispozici.')+'</p>';
  var dalsi=(j.prehled&&j.prehled.dalsi)?('<div class="card" style="background:#f2f7ee;padding:12px 18px">📋 '+t('Máte další výkresy čekající na Vaše rozhodnutí:')+' <b>'+j.prehled.dalsi+'</b> · <a href="'+esc(j.prehled.url)+'" style="color:var(--g2);font-weight:700">'+t('Zobrazit přehled všech výkresů →')+'</a></div>'):'';
  var head=dalsi+'<div class="card"><h1>'+t('Výkres')+' '+esc(j.cislo)+'</h1><div class="muted">'+esc(t(j.typName||''))+'</div>'+
    '<div class="meta"><span><b>'+t('Zákazník:')+'</b> '+esc(j.zakaznik)+'</span><span><b>'+t('Verze:')+'</b> v'+esc(j.version||'—')+'</span></div></div>';
  var pdfCard='<div class="card">'+pdf+'</div>';
  var actions='';
  if(d){
    var kdo=(d.name?esc(d.name)+' · ':'')+fdt(d.at)+(d.version?(' · '+t('Verze:')+' v'+d.version):'');
    if(d.action==='schvalit')actions='<div class="done ok"><b>'+t('✓ Výkres byl schválen a předán do výroby.')+'</b><br>'+kdo+'<div class="muted" style="margin-top:6px">'+t('Odkaz zůstává platný — výkres si tu můžete kdykoli znovu prohlédnout.')+'</div></div>';
    else actions='<div class="done rej"><b>'+t('✕ Výkres byl zamítnut.')+'</b><br>'+kdo+'<div class="muted" style="margin-top:6px">'+t('Ozve se Vám náš obchodník.')+'</div></div>';
    actions='<div class="card">'+actions+'</div>';
  }else if(j.revize){
    actions='<div class="card"><div class="done"><b>'+t(j.revizeZadani?'Zadání zakázky bylo upraveno — výkres se přepracovává.':'Připomínky jsme přijali.')+'</b><div class="muted" style="margin-top:6px">'+t('Konstrukce zapracovává úpravy — jakmile bude nová verze hotová, pošleme Vám ji ke schválení.')+'</div></div></div>';
  }else if(j.storno){
    actions='<div class="card"><div class="done rej"><b>'+t('Zakázka byla stornována.')+'</b><div class="muted" style="margin-top:6px">'+t('V případě dotazu se prosím obraťte na svého obchodníka.')+'</div></div></div>';
  }else{
    actions='<div class="card"><p class="muted" style="margin-top:0">'+t('Prohlédněte si výkres a zvolte, jak chcete pokračovat:')+'</p>'+
      '<div class="btns">'+
      '<button class="ok" onclick="show(\\'ok\\')">'+t('✓ Schválit')+'</button>'+
      '<button class="note" onclick="show(\\'note\\')">'+t('✎ Poslat připomínky')+'</button>'+
      '<button class="rej" onclick="show(\\'rej\\')">'+t('✕ Zamítnout')+'</button>'+
      '</div>'+
      '<div id="form"></div><div class="err" id="err"></div></div>';
  }
  var comm='';
  if(j.comments&&j.comments.length){
    comm='<div class="card"><b>'+t('Komunikace')+'</b>'+j.comments.map(function(c){return '<div style="margin-top:10px;border-top:1px solid var(--line);padding-top:8px"><div class="muted">'+esc(c.author)+' · '+fdt(c.at)+'</div><div>'+esc(c.text)+'</div></div>'}).join('')+'</div>';
  }
...
function ktoBlok(){
  if(!KONTAKT) return '<label>'+t('Vaše jméno')+'<input id="nm" placeholder="'+t('Jméno a příjmení')+'"></label>';
  return '<div class="kdo">'+t('Rozhodujete jako')+' <b>'+esc(KONTAKT)+'</b>'+
    ' <a href="#" onclick="jinyClovek(event)">'+t('rozhoduje někdo jiný?')+'</a></div>'+
    '<input type="hidden" id="nm" value="'+esc(KONTAKT)+'">';
}
function jinyClovek(e){
  e.preventDefault();
  var d=e.target.parentNode;
  d.outerHTML='<label>'+t('Vaše jméno')+'<input id="nm" placeholder="'+t('Jméno a příjmení')+'"></label>';
}
function show(kind){
  var f=document.getElementById('form');if(!f)return;
  if(kind==='ok')f.innerHTML=ktoBlok()+
    '<div class="chk"><input type="checkbox" id="sh"><span>'+t('Potvrzuji, že výkres odpovídá objednávce a schvaluji jej k výrobě.')+'</span></div>'+
    '<div class="btns"><button class="ok" onclick="send(\\'schvalit\\')">'+t('Schválit výkres')+'</button></div>';
  else if(kind==='note')f.innerHTML=ktoBlok()+
    '<label>'+t('Připomínky k výkresu')+'<textarea id="tx" rows="4" placeholder="'+t('Popište, co je třeba upravit…')+'"></textarea></label>'+
    '<div class="btns"><button class="note" onclick="send(\\'pripominky\\')">'+t('Odeslat připomínky')+'</button></div>';
  else f.innerHTML=ktoBlok()+
    '<label>'+t('Důvod zamítnutí')+'<textarea id="dv" rows="3" placeholder="'+t('Uveďte prosím důvod…')+'"></textarea></label>'+
    '<div class="btns"><button class="rej" onclick="send(\\'zamitnout\\')">'+t('Zamítnout výkres')+'</button></div>';
}
...
// Přehled všech výkresů jednoho klienta (stálý odkaz vázaný na e-mail klienta):
// co čeká na jeho rozhodnutí a co už rozhodl. Slovník NL sdílí s náhledem.
const PREHLED_HTML = `<!doctype html><html lang="cs"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<meta name="robots" content="noindex,nofollow">
<title>Výkresy ke schválení · ELKOPLAST</title>
<style>
:root{--g:#0e8a43;--g2:#0a6b34;--ink:#0f1512;--mut:#5b635c;--line:#e3e7e0;--bg:#eef1ec;--red:#c23636;--amber:#b06f00}
*{box-sizing:border-box}
body{margin:0;font-family:Segoe UI,Roboto,Arial,sans-serif;color:var(--ink);background:var(--bg)}
.top{background:#fff;border-bottom:1px solid var(--line);padding:12px 18px;display:flex;align-items:center;gap:12px;position:sticky;top:0;z-index:5}
.top .brand{font-weight:800;color:var(--g2);letter-spacing:.5px}
...
.card{background:#fff;border:1px solid var(--line);border-radius:14px;padding:18px;margin-bottom:16px;box-shadow:0 1px 2px rgba(0,0,0,.03)}
h1{font-size:19px;margin:0 0 4px}
h2{font-size:13px;margin:0 0 6px;color:var(--mut);font-weight:800;letter-spacing:.4px;text-transform:uppercase}
.muted{color:var(--mut);font-size:14px}
.it{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:12px 0;border-top:1px solid var(--line)}
.it:first-of-type{border-top:0}
.it .n{font-weight:800;font-size:16px}
.it .m{color:var(--mut);font-size:13px;margin-top:2px}
.btn{display:inline-block;background:var(--g);color:#fff;text-decoration:none;font-weight:700;border-radius:10px;padding:10px 14px;white-space:nowrap}
.btn:hover{background:var(--g2)}
.tag{display:inline-block;font-size:12px;font-weight:700;border-radius:999px;padding:2px 9px;background:#eef1ec;color:#33513f;vertical-align:middle}
.tag.ok{background:#e6f6ec;color:var(--g2)}.tag.rej{background:#fdecea;color:var(--red)}.tag.rev{background:#fff3d6;color:var(--amber)}
.empty{padding:16px;border-radius:12px;background:#f2f7ee;color:var(--g2);font-weight:600}
.foot{text-align:center;color:var(--mut);font-size:12px;padding:14px}
a.lnk{color:var(--g2)}
</style></head><body>
<div class="top"><span class="brand">ELKOPLAST</span><span class="muted" id="topt">Výkresy ke schválení</span></div>
<div class="wrap" id="root"><div class="card"><p class="muted">Načítám…</p></div></div>
<div class="foot" id="foot">Zabezpečený přehled · odkaz prosím nešiřte dál.</div>
<script>
${NL_LINE}
var PL={"Výkresy ke schválení":["Drawings for approval","Zeichnungen zur Freigabe","Rysunki do zatwierdzenia","Креслення на затвердження","Desene spre aprobare","Dessins à approuver"],
"Přehled výkresů čekajících na Vaše rozhodnutí":["Overview of drawings awaiting your decision","Übersicht der Zeichnungen, die auf Ihre Entscheidung warten","Przegląd rysunków oczekujących na Państwa decyzję","Огляд креслень, що очікують на ваше рішення","Lista desenelor care așteaptă decizia dumneavoastră","Liste des dessins en attente de votre décision"],
"Čeká na Vaše rozhodnutí":["Awaiting your decision","Wartet auf Ihre Entscheidung","Oczekuje na decyzję","Очікує на ваше рішення","Așteaptă decizia dumneavoastră","En attente de votre décision"],
"Žádný výkres teď nečeká na Vaše rozhodnutí.":["No drawing is currently awaiting your decision.","Derzeit wartet keine Zeichnung auf Ihre Entscheidung.","Obecnie żaden rysunek nie oczekuje na decyzję.","Наразі жодне креслення не очікує на ваше рішення.","Momentan niciun desen nu așteaptă decizia dumneavoastră.","Aucun dessin n’attend actuellement votre décision."],
"Otevřít a rozhodnout →":["Open and decide →","Öffnen und entscheiden →","Otwórz i zdecyduj →","Відкрити та вирішити →","Deschide și decide →","Ouvrir et décider →"],
"Odesláno":["Sent","Gesendet","Wysłano","Надіслано","Trimis","Envoyé"],
"Verze":["Version","Version","Wersja","Версія","Versiune","Version"],
"Již rozhodnuto":["Already decided","Bereits entschieden","Już rozstrzygnięte","Уже вирішено","Deja decis","Déjà décidé"],
"schváleno":["approved","freigegeben","zatwierdzono","затверджено","aprobat","approuvé"],
"zamítnuto":["rejected","abgelehnt","odrzucono","відхилено","respins","refusé"],
"připomínky — připravuje se nová verze":["comments sent — a new version is being prepared","Anmerkungen gesendet — neue Version in Arbeit","uwagi wysłane — przygotowywana jest nowa wersja","зауваження надіслано — готується нова версія","observații trimise — se pregătește o nouă versiune","remarques envoyées — une nouvelle version est en préparation"],
"Váš obchodník:":["Your sales representative:","Ihr Ansprechpartner im Vertrieb:","Państwa opiekun handlowy:","Ваш менеджер:","Reprezentantul dumneavoastră de vânzări:","Votre commercial :"],
"Zabezpečený přehled · odkaz prosím nešiřte dál.":["Secure overview · please do not share the link.","Gesicherte Übersicht · bitte geben Sie den Link nicht weiter.","Bezpieczny przegląd · prosimy nie udostępniać linku.","Захищений огляд · будь ласка, не поширюйте посилання.","Listă securizată · vă rugăm să nu distribuiți linkul.","Liste sécurisée · merci de ne pas transmettre le lien."]};
function p(x){ if(LANG==='cs'||!x) return x; var e=PL[x]; var i=LIDX[LANG]; return (e&&e[i])||x; }
var TOKEN=location.pathname.split('/').filter(Boolean).pop();
function esc(s){return String(s==null?'':s).replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]})}
function fdt(ms){ if(!ms) return ''; var x=new Date(ms); return x.toLocaleDateString(LANG==='cs'?'cs-CZ':(LANG==='ua'?'uk-UA':LANG)); }
function render(j){
  LANG=j.lang||'cs';
  if(LANG!=='cs'){ document.documentElement.lang=(LANG==='ua'?'uk':LANG); document.title=p('Výkresy ke schválení')+' · ELKOPLAST';
    document.getElementById('topt').textContent=p('Výkresy ke schválení'); document.getElementById('foot').textContent=p('Zabezpečený přehled · odkaz prosím nešiřte dál.'); }
  var h='<div class="card"><h1>'+p('Přehled výkresů čekajících na Vaše rozhodnutí')+'</h1><div class="muted">'+(j.zakaznik?('<b>'+t('Zákazník:')+'</b> '+esc(j.zakaznik)):'')+(j.kontakt?(' · '+esc(j.kontakt)):'')+'</div></div>';
  h+='<div class="card"><h2>'+p('Čeká na Vaše rozhodnutí')+' ('+j.cekajici.length+')</h2>';
  if(!j.cekajici.length) h+='<div class="empty">'+p('Žádný výkres teď nečeká na Vaše rozhodnutí.')+'</div>';
  j.cekajici.forEach(function(x){ h+='<div class="it"><div><div class="n">'+esc(x.cislo)+'</div><div class="m">'+esc(t(x.typName||''))+(x.verze?(' · '+p('Verze')+' v'+x.verze):'')+(x.odeslano?(' · '+p('Odesláno')+' '+fdt(x.odeslano)):'')+'</div></div><a class="btn" href="'+esc(x.url)+'">'+p('Otevřít a rozhodnout →')+'</a></div>'; });
  h+='</div>';
  if(j.vyrizene&&j.vyrizene.length){ h+='<div class="card"><h2>'+p('Již rozhodnuto')+'</h2>'; j.vyrizene.forEach(function(x){ var tg=x.stav==='schvalit'?'<span class="tag ok">'+p('schváleno')+'</span>':(x.stav==='zamitnout'?'<span class="tag rej">'+p('zamítnuto')+'</span>':'<span class="tag rev">'+p('připomínky — připravuje se nová verze')+'</span>'); h+='<div class="it"><div><div class="n">'+esc(x.cislo)+' '+tg+'</div><div class="m">'+esc(t(x.typName||''))+(x.at?(' · '+fdt(x.at)):'')+'</div></div><a class="lnk" href="'+esc(x.url)+'">→</a></div>'; }); h+='</div>'; }
  if(j.obchodnik) h+='<div class="card muted">'+p('Váš obchodník:')+' <b>'+esc(j.obchodnik.name||'')+'</b>'+(j.obchodnik.email?(' · <a class="lnk" href="mailto:'+esc(j.obchodnik.email)+'">'+esc(j.obchodnik.email)+'</a>'):'')+'</div>';
  document.getElementById('root').innerHTML=h;
}
fetch('/api/konstrukce/prehled/'+TOKEN+location.search,{cache:'no-store'}).then(function(r){return r.json().then(function(j){return {status:r.status,j:j}})}).then(function(o){
  if(o.status!==200){document.getElementById('root').innerHTML='<div class="card"><p class="muted">'+t('Odkaz není platný.')+'</p></div>';return}
  render(o.j);
}).catch(function(){document.getElementById('root').innerHTML='<div class="card"><p class="muted">'+t('Nepodařilo se načíst náhled.')+'</p></div>'});
</script></body></html>`;
